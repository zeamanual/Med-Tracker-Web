...
function Signup() {
    ...
    let validators = [firstNameChangeHandler, lastNameChangeHandler, emailChangeHandler, passwordChangeHandler]
            ...
            <Box display={'flex'} flexDirection={'column'} justifyContent='space-between' width={{ xs: '100vw', md: '60vw', lg: '50vw' }}>
                    ...
                    <Box justifyContent={'space-between'} display={'flex'} flexDirection='column'>
                        ...
                        <Box m={2}>
                            ...
                            <Box my={3} bgcolor='white'>
                                <TextField sx={{ m: 1 }} variant="standard" InputProps={{ disableUnderline: true, }} fullWidth={true} size='large' value={signupDetails.firstName}
                                    onChange={firstNameChangeHandler} label='first name' ></TextField>
                                {errorDetails.firstName.hasError ? <Alert sx={{ padding: 0, marginTop: 1 }} severity="error">{errorDetails.firstName.msg}</Alert> : <></>}
                            </Box>


                            <Box my={3} bgcolor='white'>
                                <TextField sx={{ m: 1 }} variant="standard" InputProps={{ disableUnderline: true, }} fullWidth={true} size='large' value={signupDetails.lastName}
                                    onChange={lastNameChangeHandler} label='last name' ></TextField>
                                {errorDetails.lastName.hasError ? <Alert sx={{ padding: 0, marginTop: 1 }} severity="error">{errorDetails.lastName.msg}</Alert> : <></>}
                            </Box>


                            <Box my={3} bgcolor='white'>
                                <TextField sx={{ m: 1 }} variant="standard" InputProps={{ disableUnderline: true, }} fullWidth={true} size='large' value={signupDetails.email}
                ...
                <Box height='auto' m={2} mt="auto" display={'flex'} flexDirection={'column'} alignItems='center' >
                    ...
                    <Box width={'100%'} borderRadius={30} overflow='hidden'>
                        <Button onClick={formSubmitHandler} sx={{ padding: '0.7em' }} fullWidth type='submit' variant='contained'>Sign up</Button>
                    </Box>
                </Box>
            </Box>
        </Box>
